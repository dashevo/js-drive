...
async function createDIContainer(options) {
  ...
  container.register({
    ...
    identityCacheSize: asValue(options.IDENTITY_CACHE_SIZE),
  ...
  container.register({
    ...
    identityCache: asFunction((identityCacheSize) => (
      new LRUCache(identityCacheSize)
    )).singleton(),
  });
  ...
  container.register({
    ...
    stateRepository: asFunction((
      ...
      return new CachedStateRepositoryDecorator(stateRepository, dataContractCache, identityCache);
    }).singleton(),
