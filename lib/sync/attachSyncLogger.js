...
module.exports = function attachSyncLogger(readerMediator, logger) {
  ...
  readerMediator.on(ReaderMediator.EVENTS.DAP_CONTRACT_APPLIED, (params) => {
    ...
    logger.info(
      `${contractAction} DAP Contract for ${dapId}`,
      {
        userId,
        dapId,
        reference,
  ...
  readerMediator.on(ReaderMediator.EVENTS.DAP_OBJECT_APPLIED, (params) => {
    ...
    const messages = {};
    messages[DapObject.ACTION_CREATE] = `Created DAP Object ${objectId} for ${dapId}`;
    messages[DapObject.ACTION_UPDATE] = `Updated DAP Object ${objectId} for ${dapId}`;
    messages[DapObject.ACTION_DELETE] = `Deleted DAP Object ${objectId} for ${dapId}`;
