...
class DapContractMongoDbRepository {
   ...
   */
  async find(dapId) {
    const dbId = this.createIdFromDapId(dapId);
    const result = await this.collection.findOne({ _id: dbId, isDeleted: false });
    if (!result) {
      return null;
    }
  ...
  async store(dapContract) {
    ...
    const dbId = this.createIdFromDapId(dapContractData.dapId);
