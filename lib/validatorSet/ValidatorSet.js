...
class ValidatorsSet {
   ...
   */
  selectValidatorSetHash(quorumHashes, rotationSignature) {
    const scoredHashes = this.calculateScores(quorumHashes, rotationSignature);
   ...
   */
  async getHashForCoreHeight(rotationSignature, coreBlockHeight) {
    ...
    return this.selectValidatorSetHash(validatorQuorumHashes, rotationSignature).toString('hex').reverse();
  }
}
