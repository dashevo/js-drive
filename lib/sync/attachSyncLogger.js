...
module.exports = function attachSyncLogger(readerMediator, logger) {
  ...
  readerMediator.on(ReaderMediator.EVENTS.STATE_TRANSITION_APPLIED, (params) => {
    const { block, stateTransition, packet } = params;
    logger.info(
      `Processing of State Transition ${stateTransition.hash} for block ${block.height} is finished`,
      {
        block: {
          hash: block.hash,
          height: block.height,
        ...
        event: ReaderMediator.EVENTS.STATE_TRANSITION_APPLIED,
      },
    );
  });
