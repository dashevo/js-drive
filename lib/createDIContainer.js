...
async function createDIContainer(options) {
  ...
  container.register({
    ...
    logFilePath: asValue(options.LOG_FILE_PATH),
   ...
   */
  container.register({
    loggerStreams: asFunction((logFilePath) => [
      {
    ...
    logger: asFunction((loggingLevel, loggerStreams) => pino({
      level: loggingLevel,
      prettyPrint: {
        translateTime: true,
      },
    }, pinoMultistream(loggerStreams))).singleton(),
