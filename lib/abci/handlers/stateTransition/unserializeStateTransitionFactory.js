  ...
  async function unserializeStateTransition(stateTransitionByteArray, options = {}) {
    ...
    try {
      ...
      if (e instanceof InvalidStateTransitionError) {
        const error = new InvalidArgumentGrpcError('State Transition is invalid', { errors: e.getErrors() });
    ...
    if (!result.isValid()) {
      ...
      const error = new InvalidArgumentGrpcError('State Transition is invalid', { errors: consensusErrors });
    ...
    if (!result.isValid()) {
      ...
      if (consensusErrors.length === 1 && consensusErrors[0] instanceof BalanceIsNotEnoughError) {
        error = new ResourceExhaustedGrpcError('Not enough credits', { balance: consensusErrors[0].getBalance() });

        logger.info('Insufficient funds to process state transition');
      } else {
        error = new InvalidArgumentGrpcError('State Transition is invalid', { errors: consensusErrors });
