...
function updateSimplifiedMasternodeListFactory(
  ...
  async function updateSimplifiedMasternodeList(coreHeight, options = {}) {
    ...
    if (latestRequestedHeight === 1) {
      ...
      contextLogger.debug(`SML is initialized for heights ${startHeight} to ${coreHeight}`);
      ...
      contextLogger.debug(`SML is updated for heights ${latestRequestedHeight} to ${coreHeight}`);
