...
module.exports = function attachBlockErrorsHandler(readerMediator, options) {
   ...
   */
  async function handleErrors({ error, block }) {
    // If we want to skip block processing with errors
    if (skipBlockWithErrors) {
      await this.readerMediator.emitSerial(ReaderMediator.EVENTS.BLOCK_SKIP, block);
      ...
      // Finish reading if it is the last block
      return;
    }
