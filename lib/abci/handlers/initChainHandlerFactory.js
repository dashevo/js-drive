...
function initChainHandlerFactory(
  ...
  async function initChainHandler() {
    ...
    const sml = smlStore.getSMLbyHeight(initialCoreChainLockedHeight);
    const validatorLlmqType = sml.getValidatorLLMQType();
    const lastCoreBlockHash = sml.toSimplifiedMNListDiff().blockHash;

    const validatorQuorumHash = await new ValidatorSet(sml).getHashForCoreHeight(
      Buffer.from(lastCoreBlockHash), initialCoreChainLockedHeight,
    );
