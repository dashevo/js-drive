 ...
 * @param {createDocumentDbFromStoreTransaction} createDocumentDbFromStoreTransaction
...
function beginBlockHandlerFactory(
  ...
  createDocumentDbFromStoreTransaction,
  protocolVersion,
  logger,
) {
  /**
  ...
  async function beginBlockHandler({ header }) {
    ...
    await Promise.all(
      Object.entries(previousTransactions).map(async ([name, t]) => {
        await t.start();

        if (name === 'dataContract') {
          await createDocumentDbFromStoreTransaction(t);
        }
