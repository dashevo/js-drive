...
class DapContractMongoDbRepository {
   ...
   */
  constructor(mongoDb, { serialize, deserialize }) {
  ...
  async findAllByReferenceSTHeaderHash(hash) {
    ...
    return Promise.all(
      result.map(document => this.toDapContract(document)),
    );
  }
  ...
  async store(dapContract) {
    ...
    dapContractData.data = mongo.Binary(
      this.serialize(dapContractData.data),
    );
   ...
   */
  async toDapContract(contractFromDb) {
    const contractData = Object.assign({}, contractFromDb, {
      data: await this.deserialize(contractFromDb.data.buffer),
    });
