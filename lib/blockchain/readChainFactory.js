/**
 * @param {STHeadersReader} stHeadersReader
 * @param {RpcClient} rpcClient
 * @param {SyncState} syncState
 * @param {string} sinceBlockHash
 * @returns {Promise<void>}
 */
async function resetFromMostCurrentBlock(stHeadersReader, rpcClient, syncState, sinceBlockHash) {
  // Start sync from the last synced block + 1
  if (syncState.getLastInitialSyncAt()) {
    this.setBlockHeight(syncState.getLastBlock().height + 1);
  }
  // Reset height to the current block's height
  if (sinceBlockHash) {
    const { result: { height: blockHeight } } = await rpcClient.getBlock(sinceBlockHash);
    if (blockHeight < this.getBlockHeight()) {
      stHeadersReader.stHeaderIterator.blockIterator.setBlockHeight(blockHeight);
    }
  }
  stHeadersReader.stHeaderIterator.reset(false);
}

/**
 * @param {STHeadersReader} stHeadersReader
 * @param {RpcClient} rpcClient
 * @param {SyncState} syncState
 * @param {cleanDashDrive} cleanDashDrive
 * @returns {readChain}
 */
function readChainFactory(stHeadersReader, rpcClient, syncState, cleanDashDrive) {
  let isInSync = false;

  /**
   * @typedef {Promise} readChain
   * @param {string} [sinceBlockHash]
   * @returns {Promise<void>}
   */
  async function readChain(sinceBlockHash) {
    try {
      if (isInSync) {
        return;
      }
      isInSync = true;

      await resetFromMostCurrentBlock(stHeadersReader, rpcClient, syncState, sinceBlockHash);

      await stHeadersReader.read();
      isInSync = false;
    } catch (error) {
      if (error.message === 'Block height out of range' && !syncState.isEmpty()) {
        await cleanDashDrive();

        stHeadersReader.resetToBlockHeight(1);
        syncState.clean();

        isInSync = false;

        await readChain();

        return;
      }
      isInSync = false;
      throw error;
    }
  }

  return readChain;
}

module.exports = readChainFactory;
