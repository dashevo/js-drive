...
const revertDapContractsForBlockFactory = require('../stateView/revertDapContractsForBlockFactory');
...
class SyncApp {
  ...
  /**
   * Create revertDapContractsForBlock
   *
   * @param {applyStateTransition} applyStateTransition
   * @returns {revertDapContractsForBlock}
   */
  createRevertDapContractsForBlock(applyStateTransition) {
    const mongoDb = this.getMongoClient().db(this.options.getStorageMongoDbDatabase());
    const dapContractMongoDbRepository = new DapContractMongoDbRepository(mongoDb, sanitizeData);
    return revertDapContractsForBlockFactory(
