...
module.exports = function applyStateTransitionHandlerFactory(
  ...
  async function applyStateTransitionHandler(call) {
    ...
    try {
      stPacket = await dpp.packet.createFromSerialized(Buffer.from(stPacketBinaryArray).toString('hex'));
    } catch (e) {
    ...
    try {
      ...
      throw new InvalidArgumentGrpcError('Invalid "stateTransitionHeader"', { error: e });
    }

    const result = await dpp.packet.verify(stPacket, stHeader);
    if (!result.isValid()) {
      throw new InvalidArgumentGrpcError('Invalid "stPacket" and "stateTransition"', { errors: result.getErrors() });
    }
