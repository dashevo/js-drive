...
function endBlockHandlerFactory(
  ...
  async function endBlockHandler(request) {
    ...
    if (validatorSet.rotate(height)) {
       const quorum = validatorSet.getQuorum();
    
       const quorumPublicKey = Buffer.from(quorum.quorumPublicKey, 'hex');
       const quorumHash = Buffer.from(quorum.quorumHash, 'hex')

       validatorSetUpdate = new ValidatorSetUpdate({
         validatorUpdates: newValidatorsUpdates,
         thresholdPublicKey: new PublicKey({
           bls12381: quorumPublicKey,
         }),
       ...
       consensusLogger.debug(
        {
          quorumHash: quorumHash.toString('hex'),
        },
        `Validator Set switched to ${quorumHash.toString('hex'))} quorum`,
      );
    }
    */
