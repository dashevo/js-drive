...
describe('Sync interruption and resume between Dash Drive and Dash Core', function main() {
  ...
  it('Dash Drive should save sync state and continue from saved point after resume', async () => {
    ...
    let lsResult = await dashDriveSecond.ipfs.getApi().pin.ls();
    ...
    for (let i = 0; i < 60; i++) {
      lsResult = await dashDriveSecond.ipfs.getApi().pin.ls();
    ...
    await dashDriveSecond.driveSync.stop();
    ...
    const rmPromises = Promise
      .all(pinnedHashes.map(hash => dashDriveSecond.ipfs.getApi().pin.rm(hash)));
    await rmPromises;

    // 9. Start Dash Drive on 2nd node
    await dashDriveSecond.driveSync.start();
