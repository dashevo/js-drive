...
async function createDIContainer(options) {
  ...
  container.register({
    ...
    queryHandlerRouter: asFunction((
      ...
      router.on('GET', '/identities/by-id', identityQueryHandler);
      router.on('GET', '/dataContracts/by-id', dataContractQueryHandler);
      router.on('GET', '/dataContracts/documents/by-type', documentQueryHandler);
