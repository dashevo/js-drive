...
const InternalAbciError = require('../errors/InternalAbciError');
...
function beginBlockHandlerFactory(
  ...
  async function beginBlockHandler(request) {
    ...
    if (version.app === undefined) {
      throw new InternalAbciError(
        new Error('app version was not present in consensus parameters'),
      );
    }
