...
const applyReorgForDapContractFactory = require('../stateView/applyReorgForDapContractFactory');
...
class SyncApp {
  ...
  /**
   * Create applyReorgForDapContract
   *
   * @param {applyStateTransition} applyStateTransition
   * @returns {applyReorgForDapContract}
   */
  createApplyReorgForDapContract(applyStateTransition) {
    const mongoDb = this.getMongoClient().db(this.options.getStorageMongoDbDatabase());
    const dapContractMongoDbRepository = new DapContractMongoDbRepository(mongoDb, sanitizeData);
    return applyReorgForDapContractFactory(
