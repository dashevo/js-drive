...
module.exports = function attachSyncLogger(readerMediator, logger) {
  ...
  readerMediator.on(ReaderMediator.EVENTS.DAP_CONTRACT_APPLIED, (params) => {
    const { userId, dapId, reference, contract } = params;
    logger.info(
      `Updated DAP Contract for ${dapId}`,
      {
        userId,
        dapId,
        reference,
  ...
  readerMediator.on(ReaderMediator.EVENTS.DAP_CONTRACT_REVERTED, (params) => {
    const { userId, dapId, reference, contract, previousVersion } = params;
    logger.info(
      `Reverted DAP Contract for ${dapId} from ${contract.dapver} to ${previousVersion.version}`,
      {
        userId,
        dapId,
        reference,
  ...
  readerMediator.on(ReaderMediator.EVENTS.DAP_OBJECT_APPLIED, (params) => {
    const { userId, dapId, reference, object } = params;

    let message;
    switch (object.act) {
      case DapObject.ACTION_CREATE: {
        message = `Created DAP Object for ${dapId}`;
        break;
      }
      case DapObject.ACTION_UPDATE: {
        message = `Updated DAP Object for ${dapId}`;
        break;
      }
      case DapObject.ACTION_DELETE: {
        message = `Deleted DAP Object for ${dapId}`;
        break;
      }
      default:
    }
  ...
  readerMediator.on(ReaderMediator.EVENTS.DAP_OBJECT_REVERTED, (params) => {
    const { userId, objectId, reference, object, previousRevision } = params;
    logger.info(
  ...
  readerMediator.on(ReaderMediator.EVENTS.DAP_OBJECT_MARKED_DELETED, (params) => {
    const { userId, objectId, reference, object } = params;
    logger.info(
