...
class DapContractMongoDbRepository {
  ...
  /**
   * Find list of DapContract by `reference.blockHash`
   *
   * @param {string} blockHash
   * @returns {Promise<[DapContract]|null>}
   */
  async findAllByReferenceBlockHash(blockHash) {
    const result = await this.collection.find({ 'reference.blockHash': blockHash }).toArray();
    if (!result) {
      return null;
    }
  ...
  /**
   * Delete DapContract entity by reference.blockHash
   *
   * @param {string} blockHash
   * @returns {Promise}
   */
  async deleteByReferenceBlockHash(blockHash) {
    return this.collection.deleteOne({ 'reference.blockHash': blockHash });
  }
