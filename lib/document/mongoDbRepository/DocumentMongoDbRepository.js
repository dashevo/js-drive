...
class DocumentMongoDbRepository {
  ...
  async createIndices(indices) {
    const partialFilterExpression = indices
      .map((index) => Object.keys(index.key))
      .reduce((allFieldNames, indicesFieldNames) => (
        [...allFieldNames, ...indicesFieldNames]
      ), [])
      .reduce((expression, nextIndexedFieldName) => ({
        ...expression,
        [nextIndexedFieldName]: { $exists: true },
      }), {});
