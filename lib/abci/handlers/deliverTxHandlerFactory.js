...
function deliverTxHandlerFactory(
  ...
  async function deliverTxHandler({ tx: stateTransitionByteArray }) {
    ...
    logger.info(logInfoObject, `Deliver state transition ${stHash} from block #${blockHeight}`);
    ...
    try {
      ...
      if (e instanceof InvalidArgumentAbciError) {
        logger.info(logInfoObject, 'State transition structure is invalid');
        logger.debug(logInfoObject, e);
      }
    ...
    if (!result.isValid()) {
      logger.info(logInfoObject, 'State transition data is invalid');
      logger.debug(logInfoObject, {
    ...
    switch (stateTransition.getType()) {
      case stateTransitionTypes.DATA_CONTRACT_CREATE: {
        ...
        logger.info(logInfoObject, `Data contract created with id: ${dataContract.getId()}`);

        break;
      }
      case stateTransitionTypes.IDENTITY_CREATE: {
        const identityId = stateTransition.getIdentityId();
        logger.info(logInfoObject, `Identity created with id: ${identityId}`);
        break;
      }
      case stateTransitionTypes.DOCUMENTS_BATCH: {
        stateTransition.getTransitions().forEach((transition) => {
          ...
          logger.info(logInfoObject, `Document ${description} with id: ${transition.getId()}`);
        });
        break;
      }
      default:
