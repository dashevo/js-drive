...
class STHeadersReader extends Emittery {
   ...
   *   blockProcessingRetries: number,
   *   skipBlockWithErrors: boolean }} options
   */
  constructor(stHeaderIterator, state, options = {}) {
    super();
  ...
  async read() {
    ...
    let blockProcessingRetryCount = 0;
      ...
      try { // Catch errors for block sync
        ...
        if (currentBlock !== this.previousIteratedBlock) { // Iterate over new block
          ...
          if (this.previousIteratedBlock) { // Previous block end
            await this.handlePreviousBlock(this.previousIteratedBlock);
          }

          this.previousIteratedBlock = currentBlock;

          // Handle new block if we are not done reading
          if (!done) {
            const isHandled = await this.handleNewBlock(currentBlock);
            if (!isHandled) {
        ...
        if (error instanceof InvalidBlockHeightError) {
          if (error.blockHeight === this.options.initialBlockHeight) {
            throw error;
          }
          ...
          this.restartIterator(this.options.initialBlockHeight);
        ...
        if (this.options.skipBlockWithErrors && !done) {
          // Move iterator to the next item
          this.restartIterator(currentBlock.height + 1);
        ...
        if (blockProcessingRetryCount > this.options.blockProcessingRetries) {
          throw error;
        }

        blockProcessingRetryCount += 1;
   ...
   */
  async handlePreviousBlock(block) {
    // Mark block as read
    this.state.addBlock(block);

    await this.emitSerial(STHeadersReader.EVENTS.BLOCK_END, block);
  }
