class BlockExecutionContext {
  ...
  /**
   * Clones current instance into separate one
   *
   * @returns {BlockExecutionContext}
   */
  clone() {
    const cloned = new BlockExecutionContext();

    cloned.dataContracts = this.dataContracts;
    cloned.cumulativeFees = this.cumulativeFees;
    cloned.header = this.header;
    cloned.validTxs = this.validTxs;
    cloned.invalidTxs = this.invalidTxs;
    cloned.consensusLogger = this.consensusLogger;
