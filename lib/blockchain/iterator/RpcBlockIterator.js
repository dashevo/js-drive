...
class RpcBlockIterator extends Emittery {
  ...
  setBlockHeightFromSyncState(syncState) {
  ...
  async resetFromCurrentBlockHeight(sinceBlockHash) {
    if (sinceBlockHash) {
      const { result: { height: blockHeight } } = await this.rpcClient.getBlock(sinceBlockHash);
