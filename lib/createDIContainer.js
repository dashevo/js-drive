...
const wrapHandlerWithLoggerFactory = require('./abci/logging/wrapHandlerWithLoggerFactory');
...
async function createDIContainer(options) {
   ...
   */
  container.register({
    loggerFormat: asFunction((loggingLevel) => winston.format.printf(
      ({
        level: levelString, message, timestamp, stack,
      }) => {
        if (loggingLevel === 'debug' || loggingLevel === 'silly') {
          let printMessage = `[${timestamp}] [${levelString}] ${message}`;
    ...
    logger: asFunction((loggingLevel, loggerFormat) => winston.createLogger({
      level: loggingLevel,
    ...
    handlerLogger: asFunction((loggingLevel, loggerFormat) => winston.createLogger({
      level: loggingLevel,
      format: winston.format.combine(
        winston.format.timestamp(),
        winston.format.errors({ stack: true }),
  ...
  container.register({
    ...
    wrapHandlerWithLogger: asFunction(wrapHandlerWithLoggerFactory).singleton(),
    ...
    abciHandlers: asFunction((
      ...
      info: wrapHandlerWithLogger(infoHandler),
      checkTx: wrapHandlerWithLogger(wrapInErrorHandler(checkTxHandler)),
      beginBlock: wrapHandlerWithLogger(beginBlockHandler),
      deliverTx: wrapHandlerWithLogger(wrapInErrorHandler(deliverTxHandler)),
      commit: wrapHandlerWithLogger(commitHandler),
      query: wrapHandlerWithLogger(wrapInErrorHandler(queryHandler)),
    })).singleton(),
  });
