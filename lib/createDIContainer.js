...
const DocumentDbTransaction = require('./document/DocumentsCombinedTransaction');
...
const BlockExecutionDBTransactions = require('./blockExecution/BlockExecutionStoreTransactions');
const BlockExecutionTransactionStore = require('./blockExecution/PreviousBlockExecutionStoreTransactionsRepository');
...
const updateMongoDbFromStoreTransactionFactory = require('./document/populateMongoDbTransactionFromObjectFactory');
const createDocumentDbFromStoreTransactionFactory = require('./dataContract/createDocumentDbFromStoreTransactionFactory');
 ...
 * @param {string} PREVIOUS_COMMON_STORE_MERK_DB_FILE
 ...
 * @param {string} options.BLOCK_EXECUTION_TRANSACTIONS_FILE
...
async function createDIContainer(options) {
  ...
  container.register({
    ...
    blockExecutionTransactionFile: asValue(options.BLOCK_EXECUTION_TRANSACTIONS_FILE),
  ...
  container.register({
    ...
    // )).singleton(),
    // previousIdentitiesTransaction: asFunction((previousIdentitiesStore) => (
    //   previousIdentitiesStore.createTransaction()
    // )).singleton(),
    // publicKeyToIdentityIdTransaction: asFunction((publicKeyToIdentityIdStore) => (
    //   publicKeyToIdentityIdStore.createTransaction()
    // )).singleton(),
    // previousPublicKeyToIdentityIdTransaction: asFunction((previousPublicKeyToIdentityIdStore) => (
    //   previousPublicKeyToIdentityIdStore.createTransaction()
    // )).singleton(),
  ...
  container.register({
    ...
    // )).singleton(),
    // previousDataContractsTransaction: asFunction((previousDataContractsStore) => (
    //   previousDataContractsStore.createTransaction()
    // )).singleton(),
    ...
    createDocumentDbFromStoreTransaction: asFunction((
      previousDocumentDatabaseManager,
      transactionalDpp,
    ) => (
      createDocumentDbFromStoreTransactionFactory(
        previousDocumentDatabaseManager,
        transactionalDpp,
      )
    )).singleton(),
  ...
  container.register({
    ...
    ) => (new DocumentStoreRepository(previousDocumentsStore))).singleton(),
    ...
    // previousDocumentMongoDBTransaction: asClass(MongoDBTransaction).singleton(),
    // documentsStoreTransaction: asFunction((documentsStore) => (
    //   documentsStore.createTransaction()
    // )).singleton(),
    // previousDocumentsStoreTransaction: asFunction((previousDocumentsStore) => (
    //   previousDocumentsStore.createTransaction()
    // )).singleton(),
    // documentsDbTransaction: asClass(DocumentDbTransaction).singleton(),
    // previousDocumentsDbTransaction: asFunction((
    //   previousDocumentsStoreTransaction,
    //   previousDocumentMongoDBTransaction,
    // ) => (
    //   new DocumentDbTransaction(
    //     previousDocumentsStoreTransaction,
    //     previousDocumentMongoDBTransaction,
    //   )
    // )).singleton(),
    ...
    updateMongoDbFromStoreTransaction: asFunction((
      getPreviousDocumentMongoDBDatabase,
      transactionalDpp,
    ) => (
      updateMongoDbFromStoreTransactionFactory(
        getPreviousDocumentMongoDBDatabase,
        transactionalDpp,
      )
    )),
  ...
  container.register({
    ...
    previousChainInfoRepository: asFunction((
      previousCommonStore,
    ) => (new ChainInfoCommonStoreRepository(previousCommonStore))).singleton(),
  });
  ...
  container.register({
    ...
    blockExecutionTransactionStore: asClass(BlockExecutionTransactionStore).singleton(),
    blockExecutionTransactionDB: asFunction((blockExecutionTransactionFile) => {
      return new FileDb(blockExecutionTransactionFile);
    }).singleton(),
