...
class ApiApp {
  ...
  constructor(options) {
    ...
    this.dapContractMongoDbRepository = null;
  }
  ...
  createCheckSyncStateHttpMiddleware() {
    ...
    return getCheckSyncStateHttpMiddleware(
      ...
      this.options.getSyncChainSheckInterval(),
    );
  }

  createDapContractsMongoDbRepository() {
    if (this.dapContractMongoDbRepository) {
      const mongoDb = this.mongoClient.db(this.options.getStorageMongoDbDatabase());
      this.dapContractMongoDbRepository = new DapContractMongoDbRepository(mongoDb, sanitizeData);
    }

    return this.dapContractMongoDbRepository;
  }
  ...
  createAddSTPacketMethod() {
    const addSTPacket = addSTPacketFactory(
      this.ipfsAPI,
      this.createDapContractsMongoDbRepository(),
    );
