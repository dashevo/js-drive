...
module.exports = function applyStateTransitionHandlerFactory(
  ...
  async function applyStateTransitionHandler(call) {
    ...
    const blockHash = Buffer.from(request.getBlockHash()).toString('hex');
    const stPacketBinary = Buffer.from(request.getStateTransitionPacket());
    const stHeaderBinary = Buffer.from(request.getStateTransitionHeader());
