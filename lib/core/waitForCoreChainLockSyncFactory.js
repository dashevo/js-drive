 ...
 *
 * @param {OldZMQClient} coreZMQClient
...
function waitForCoreChainLockSyncFactory(
  ...
  async function waitForCoreChainLockSync() {
    coreZMQClient.subscribe(OldZMQClient.TOPICS.rawchainlock);
    ...
    coreZMQClient.on(OldZMQClient.TOPICS.rawchainlock, async (rawChainLockMessage) => {
    ...
    coreZMQClient.subscribe(OldZMQClient.TOPICS.hashblock);
