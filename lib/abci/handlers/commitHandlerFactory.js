 ...
 * @param {DocumentsDatabaseManager} documentsDatabaseManager
 *
  ...
  async function commitHandler() {
    ...
    try {
      ...
      for (const dataContract of blockExecutionState.getDataContracts()) {
        await documentsDatabaseManager.create(dataContract);
      }
      ...
      for (const dataContract of blockExecutionState.getDataContracts()) {
        await documentsDatabaseManager.drop(dataContract);
      }
