...
const InvalidDateFieldError = require('./errors/InvalidDateFieldError');
...
function validateQueryFactory(
  ...
  function validateQuery(query, documentSchema) {
    ...
    if (query.where) {
      ...
      // Check date fields
      query.where
        .filter(([fieldName]) => dateFieldNames.includes(fieldName))
        .forEach(([fieldName, , fieldValue]) => {
          if (!Number.isInteger(fieldValue) || fieldValue < 0) {
            result.addError(new InvalidDateFieldError(fieldName));
          }
        });
