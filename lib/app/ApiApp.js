...
const parseBodyFactory = require('../api/middlewares/parseBodyFactory');
const bypassFactory = require('../api/middlewares/bypassFactory');
...
class ApiApp {
  ...
  /**
   * Create parse body middleware
   *
   * @returns {parseBody}
   */
  createParseBody() {
    return parseBodyFactory();
  }
   ...
   * @returns {bypass}
   */
  createBypass(fn, methods) {
    return bypassFactory(fn, methods);
  }
