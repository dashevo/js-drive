...
class RootTree {
  ...
  getRootHash() {
    const leafHashesAreEmpty = this.leafHashes.find(
      (hash) => Buffer.compare(hash, Buffer.alloc(hash.length)) !== 0,
    ) === undefined;
   ...
   * @return {Array.<{position: string, data: Buffer}>}
   */
  getProof(leaf) {
  ...
  getFullProof(leaf, leafKeys) {
    ...
    const rootTreeProof = convertRootTreeProofToBuffer(this.getProof(leaf));
