 ...
 * @return {string}
 */
function createDocumentIdFromSVDocumentId(id) {
  const svDocumentIdBuffer = Buffer.from(id, 'hex');

  return bs58.encode(svDocumentIdBuffer);
}
 ...
 * @return {string}
 */
function createDocumentIdFromSVDocument(svDocument) {
  const id = svDocument.getDocument().getId();

  return createDocumentIdFromSVDocumentId(id);
}
...
class SVDocumentMongoDbRepository {
   ...
   */
  async find(id) {
    const documentId = createDocumentIdFromSVDocumentId(id);
  ...
  store(svDocument) {
    return this.mongoClient.updateOne(
      { _id: createDocumentIdFromSVDocument(svDocument) },
