...
class ValidatorQuorums {
  ...
  constructor(simplifiedMasternodeList,
    ...
    this.sml = this.simplifiedMasternodeList.getStore().getCurrentSML();
  }

  async rotate(height, rotationEntropy) {
    // validator set is rotated every ROTATION_BLOCK_INTERVAL blocks
    if (height % ROTATION_BLOCK_INTERVAL === 0) {
      this.sml = this.simplifiedMasternodeList.getStore().getSMLbyHeight(height);
