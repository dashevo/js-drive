...
class DocumentMongoDbRepository {
  ...
  createDppDocument({
    ...
    const binaryProperties = this.dataContract.getBinaryProperties(this.documentType);

    Object.entries(binaryProperties).forEach(([propertyPath, propertyValue]) => {
      if (propertyValue.contentMediaType === 'application/x.dash.dpp.identifier') {
