...
class DocumentsDbTransaction {
   ...
   */
  toPlainObject() {
    return this.storeTransaction.toPlainObject();
  }

  /**
   * Update create and delete operations from plain object
   *
   * @param {{
   *   createOps: Object,
   *   deleteOps: string[],
   ...
   * @return {void}
   */
  updateFromPlainObject(plainObject) {
    this.storeTransaction.updateFromPlainObject(plainObject);
  }

  /**
   * Execute store operations within MongoDB transaction
   *
   * @return {Promise<void>}
   */
  async updateMongoDbFromStoreTransaction() {
    if (!this.isStarted()) {
      throw new DocumentsDBTransactionIsNotStartedError();
    }

    const createOperations = this.storeTransaction.db.data.entries;
