...
function validateOrderByFields(
  ...
  // leave only indices that contain all order by fields
  ...
  return orderByConditions
    // disallow sorting by multikey index
    .filter(([field]) => {
      if (field.indexOf('.') > -1) {
        return true;
      }

      return !compiledDataContractIndexFields
        .find(index => index
          .find(([indexField], i) => {
            if (indexField !== field) {
              return false;
            ...
            // part of compound index before our order by field
            const compoundIndexBefore = index.slice(0, i);

            // part of compound index after our order by field
            const compoundIndexAfter = index.slice(i, index.length);

            // check that all index fields of this part is inside where or orderBy conditions
            ...
            // check that all index fields of this part is inside orderBy conditions
