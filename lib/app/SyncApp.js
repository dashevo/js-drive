...
const revertDapContractsForHeaderFactory = require('../stateView/revertDapContractsForHeaderFactory');
...
class SyncApp {
  ...
  /**
   * Create revertDapContractsForHeader
   *
  ...
  createRevertDapContractsForHeader(applyStateTransition) {
    const mongoDb = this.getMongoClient().db(this.options.getStorageMongoDbDatabase());
    const dapContractMongoDbRepository = new DapContractMongoDbRepository(mongoDb, sanitizeData);
    return revertDapContractsForHeaderFactory(
