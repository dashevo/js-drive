...
// TODO why it knows about readerMediator?
...
module.exports = function readBlockchainFactory(reader, readerMediator, rpcClient) {
  ...
  async function readBlockchain() {
    ...
    let height = readerMediator.getInitialBlockHeight();
    ...
    if (height < currentBlockCount) {
      ...
      await readerMediator.emitSerial(BlockchainReaderMediator.EVENTS.OUT_OF_BOUNDS, {
      ...
      if (lastSyncedBlock.height === currentBlockCount) {
        ...
        if (lastSyncedBlock.hash === blockHash) {
          await readerMediator.emitSerial(
            BlockchainReaderMediator.EVENTS.FULLY_SYNCED,
            currentBlockCount,
          );
    ...
    await this.readerMediator.emitSerial(
      BlockchainReaderMediator.EVENTS.BEGIN,
      height,
    );
    ...
    await this.readerMediator.emitSerial(
      BlockchainReaderMediator.EVENTS.END,
      lastSyncedHeight,
    );
  }
