...
class ValidatorSet {
   ...
   * @param {string} masternodeProTxHash
   */
  constructor(
    ...
    this.masternodeProTxHash = masternodeProTxHash;
  ...
  async switchToRandomQuorum(sml, coreHeight, rotationEntropy) {
    ...
    const isCurrentNodeMember = !!quorumMembers
      .find((member) => member.valid && member.proTxHash === this.masternodeProTxHash);

    this.validators = quorumMembers
      .filter((member) => member.valid)
      .map((member) => Validator.createFromQuorumMember(member, isCurrentNodeMember));
  }
}
