...
function commitHandlerFactory(
  ...
  async function commitHandler() {
    ...
    const logInfoObject = {
      blockHeight,
      // logging app hash to know it before commit processed
      lastBlockAppHash: rootTree.getRootHash(),
    };

    logger.info(logInfoObject, `Block commit #${blockHeight}`);
    ...
    // updating app hash to know it after commit processed
    logInfoObject.lastBlockAppHash = rootTree.getRootHash();
    ...
    logger.info(
      logInfoObject,
      `Finalizing block commit for a block #${blockHeight} with app hash ${rootTree.getRootHash()}`,
    );
