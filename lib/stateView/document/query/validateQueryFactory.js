...
function validateQueryFactory(
  ...
  function validateQuery(query, documentSchema) {
    ...
    if (query.where) {
      ...
      result.addError(
        ...validateIndexedFields(dataContractIndexFields, query.where)
          .map(field => new NotIndexedFieldError(field)),
      );

      // @TODO Add validation of sorting
      // @TODO disallow sorting on wrong directions by compound indexes
      // @TODO disallow sorting by multikey index
