...
module.exports = function attachSyncLogger(readerMediator, logger) {
  ...
  readerMediator.on(ReaderMediator.EVENTS.DOCUMENT_APPLIED, (params) => {
    ...
    const messages = {
      [SVDocument.ACTION_CREATE]: `Created Document ${objectId} for ${contractId}`,
      [SVDocument.ACTION_UPDATE]: `Updated Document ${objectId} for ${contractId}`,
      [SVDocument.ACTION_DELETE]: `Deleted Document ${objectId} for ${contractId}`,
    };
  ...
  readerMediator.on(ReaderMediator.EVENTS.DOCUMENT_REVERTED, (params) => {
    ...
    logger.info(
      `Reverted Document ${objectId} from ${object.rev} to ${previousRevision.getRevision()}`,
      {
        ...params,
        previousRevision: previousRevision.toJSON(),
  ...
  readerMediator.on(ReaderMediator.EVENTS.DOCUMENT_MARKED_DELETED, (params) => {
    ...
    logger.info(
      `Document ${objectId} marked as deleted`,
      {
        ...params,
