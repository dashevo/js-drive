...
const NotSupportedAppVersionError = require('../errors/AbciError');
...
function beginBlockHandlerFactory(
  ...
  async function beginBlockHandler({ header }) {
    ...
    if (header.getAppVersion() > blockchainState.getAppVersion()) {
      throw new NotSupportedAppVersionError(
        header.getAppVersion(),
        blockchainState.getAppVersion(),
      );
    }
