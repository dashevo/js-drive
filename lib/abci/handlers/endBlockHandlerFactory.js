...
function endBlockHandlerFactory(
  ...
  async function endBlockHandler({ height }) {
    ...
    if (height % ROTATION_BLOCK_INTERVAL === 0) {
      ...
      newValidatorSet = await new Validators(smlStore).getValidatorSetForCoreHeight(
        Buffer.from(rotationEntropy), coreChainLock.height, false,
      );

      const validators = await getValidatorSetInfo(smlStore.getCurrentSML().getValidatorLLMQType(), newValidatorSet.quorumHash);
