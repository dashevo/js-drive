const MongoDbInstanceFactory = require('../mongoDb/MongoDbInstanceFactory');
const startIPFSInstance = require('../IPFS/startIPFSInstance');
const startDashCoreInstance = require('../dashCore/startDashCoreInstance');
const DashDriveInstanceFactory = require('./DashDriveInstanceFactory');
...
startDashDriveInstance.many = async function many(number) {
  ...
  for (let i = 0; i < number; i++) {
    ...
    const mongoDbInstance = MongoDbInstanceFactory.create();
    ...
    const ENV = [
      ...
      `DASHCORE_JSON_RPC_PORT=${dashCoreInstance.container.options.getMainPort()}`,
      `DASHCORE_JSON_RPC_USER=${dashCoreInstance.container.options.getRpcUser()}`,
      `DASHCORE_JSON_RPC_PASS=${dashCoreInstance.container.options.getRpcPassword()}`,
      `STORAGE_IPFS_MULTIADDR=/ip4/${apiHost}/tcp/${apiPort}`,
      `STORAGE_MONGODB_URL=mongodb://${mongoDbInstance.getIp()}`,
    ];
    const dashDriveInstance = DashDriveInstanceFactory.create({
      ENV,
    });
