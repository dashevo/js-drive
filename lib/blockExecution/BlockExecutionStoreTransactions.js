...
const DocumentsCombinedTransaction = require('../document/DocumentsCombinedTransaction');
...
class BlockExecutionStoreTransactions {
   ...
   * @param {connectToDocumentMongoDB} connectToDocumentMongoDB
   */
  constructor(
    commonStore,
    identitiesStore,
    ...
    const documentsTransaction = new DocumentsCombinedTransaction(
  ...
  getTransaction(name) {
    if (!this.transactions[name]) {
      throw new Error(`transaction ${name} not defined`);
    }
