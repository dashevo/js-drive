...
/**
 * @param {SVDocument} svObject
 * @return {string}
 */
function createDocumentIdFromSVDocument(svObject) {
  const id = svObject.getDocument().getId();
...
class SVDocumentMongoDbRepository {
  ...
  store(svObject) {
    return this.mongoClient.updateOne(
      { _id: createDocumentIdFromSVDocument(svObject) },
