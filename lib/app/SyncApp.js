...
const revertDapContractsForStateTransitionFactory = require('../stateView/revertDapContractsForStateTransitionFactory');
...
class SyncApp {
   ...
   */
  createRevertDapContractsForStateTransition(applyStateTransition) {
    const mongoDb = this.getMongoClient().db(this.options.getStorageMongoDbDatabase());
    const dapContractMongoDbRepository = new DapContractMongoDbRepository(mongoDb, sanitizeData);
