class DriveStateRepository {
  ...
  async storeIdentity(identity) {
    ...
    const storeMapPromises = identity.getPublicKeys()
      .map((publicKey) => (
        this.publicKeyIdentityIdMapLevelDBRepository.store(
          publicKey.getData(), identity.getId(), transaction,
        )
      ));

    await Promise.all(storeMapPromises);
  }
