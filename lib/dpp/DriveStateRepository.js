...
class DriveStateRepository {
  ...
  async verifyInstantLock(instantLock) {
    ...
    console.log('verifyInstantLock header', header);
    console.log('verifyInstantLock InstantLock', instantLock.toJSON());
    ...
    if (!verifyLLMQSignaturesWithCoreFeatureFlag || !verifyLLMQSignaturesWithCoreFeatureFlag.get('enabled')) {
      ...
      console.log('verifyInstantLock instantLock.verify');
    ...
    try {
      ...
      console.log('verifyInstantLock coreRpcClient.verifyIsLock result', isVerified);

      return isVerified;
    } catch (e) {
      console.log('verifyInstantLock coreRpcClient.verifyIsLock error', e);
