...
const BlockExecutionState = require('./chainInfo/blockExecution/BlockExecutionState');
const ChainInfoLevelDBRepository = require('./chainInfo/ChainInfoLevelDBRepository');
const BlockExecutionDBTransactions = require('./chainInfo/blockExecution/BlockExecutionDBTransactions');
 ...
 * @param {string} options.CHAIN_INFO_LEVEL_DB_FILE
...
async function createDIContainer(options) {
  ...
  container.register({
    ...
    chainInfoLevelDBFile: asValue(options.CHAIN_INFO_LEVEL_DB_FILE),
   ...
   */
  container.register({
    chainInfoLevelDB: asFunction((chainInfoLevelDBFile) => (
      level(chainInfoLevelDBFile, { keyEncoding: 'binary', valueEncoding: 'binary' })
    )).disposer((levelDB) => levelDB.close())
      .singleton(),

    chainInfoRepository: asClass(ChainInfoLevelDBRepository).singleton(),
  });
