...
class STHeadersReader extends Emittery {
  ...
  async onBlockHandler(currentBlock) {
    ...
    if (this.isWrongSequence(currentBlock, previousBlock)) {
      ...
      if (previousBlock && previousBlock.height > currentBlock.height) {
        await this.emitSerial('block', previousBlock);

        return this.onBlockHandler(previousBlock);
      }

      return this.restartIterator(previousBlock.height);
    }
