...
const DocumentsDatabaseManager = require('./document/mongoDbRepository/DocumentsDatabaseManager');
...
const documentsQueryHandlerFactory = require('./abci/handlers/query/documentsQueryHandlerFactory');
...
async function createDIContainer(options) {
  ...
  container.register({
    ...
    documentsDatabaseManager: asFunction((
      createDocumentRepository,
      convertToMongoDbIndices,
      getDocumentsDatabase,
    ) => new DocumentsDatabaseManager(
      createDocumentRepository,
      convertToMongoDbIndices,
      getDocumentsDatabase,
    )),
    documentsTransaction: asFunction(documentsMongoDBClient => (
      new MongoDBTransaction(documentsMongoDBClient)
    )),
  ...
  container.register({
    ...
    documentsQueryHandler: asFunction(documentsQueryHandlerFactory).singleton(),

    queryHandlerRouter: asFunction((
      identityQueryHandler,
      dataContractQueryHandler,
      documentsQueryHandler,
    ) => {
      ...
      router.on('GET', '/dataContracts/:contractId/documents/:type', documentsQueryHandler);
