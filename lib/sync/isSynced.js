...
async function isDriveFullySynced(syncInfo) {
  const lastSyncedBlockHash = syncInfo.getLastSyncedBlockHash();
  const lastChainBlockHash = syncInfo.getLastChainBlockHash();

  return !!lastSyncedBlockHash && !!lastChainBlockHash
    && lastSyncedBlockHash === lastChainBlockHash;
}
