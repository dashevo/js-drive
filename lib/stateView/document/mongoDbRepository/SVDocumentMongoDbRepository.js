...
class SVDocumentMongoDbRepository {
   ...
   */
  constructor(db, convertWhereToMongoDbQuery, validateQuery, documentType) {
    this.db = db;
    this.mongoClient = db.collection(`documents_${documentType}`);
    this.convertWhereToMongoDbQuery = convertWhereToMongoDbQuery;
    this.validateQuery = validateQuery;
    this.documentType = documentType;
  }

  async createCollection(documentType) {
    return this.db.createCollection(`documents_${documentType}`);
  }
